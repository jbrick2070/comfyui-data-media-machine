        ...
        if style == "la_morning_report":
            ...
            if cong > 70:
                parts.append(f"Heavy traffic across the basin. "
                             f"Metro buses averaging just {avg_spd:.0f} miles per hour.")
            elif cong > 40:
                parts.append(f"Moderate traffic. Buses moving at {avg_spd:.0f} on routes {route_str}.")
            else:
                parts.append(f"Roads are looking good. Buses flowing at {avg_spd:.0f} miles per hour.")
            if heading_div > 0.7:
            ...
            if cong > 70:
                return (
                    ...
                    f"Buses crawling at {avg_spd:.0f}. "
                    f"{buses} of them out there, going nowhere fast."
                )
            elif cong < 20:
                return f"The roads are empty. {avg_spd:.0f} miles an hour, wide open. Suspicious."
            else:
                return f"Traffic's moving. {avg_spd:.0f} on the surface streets. Normal chaos."

        elif style == "old_time_radio_hour":
            if cong > 60:
                return (
                    f"Motorists are advised that traffic conditions are quite congested this evening. "
                    f"The streetcars report an average speed of merely {avg_spd:.0f} miles per hour."
                )
            else:
                return f"The motorways are moving at a reasonable {avg_spd:.0f} miles per hour."
            ...
            return (
                ...
                f"The flow suggests {flow}."
            )

        elif style == "surreal_dispatch":
            return (
                f"{buses} buses dream at {avg_spd:.0f} miles per hour. "
